"""Taken and adapted from Pixel2Mesh"""
...
class VGG16P2M(nn.Module):
    ...
    def __init__(self, n_classes_input=3, pretrained=False):
        ...
        if pretrained:
            if not torch.cuda.is_available():
                state_dict = torch.load('src/runs/checkpoints/vgg_pretrained.pth', map_location=torch.device('cpu'))
            else:
                state_dict = torch.load('src/runs/checkpoints/vgg_pretrained.pth')
            state_dict = self.transform_state_dict(state_dict['model'])
    ...
    def transform_state_dict(self, state_dict):
        new_state_dict = {}
        for key, value in state_dict.items():   # iter on both keys and values
            if key.startswith('nn_encoder'):
                new_state_dict[key.split("r.")[1]] = state_dict[key]
    ...
    def forward(self, img):
        ...
        return {
            "geometry_feature": [img0, img1, img2],
            "semantic_feature": [img2, img3, img4, img5]
        }
